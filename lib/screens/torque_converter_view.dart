...
class _TorqueConverterViewState extends State<TorqueConverterView> {
  ...
  String fromUnit = 'Kilogram-force meter';
  String fromSymbol = 'kgf·m';
  String fromValue = '1';
  ...
  // --- MATH LOGIC: Base is 1 Newton meter (N·m) ---
  final Map<String, double> torqueConversionRates = {
    'Newton meter': 1.0,
    'Newton millimeter': 0.001,
    'Newton centimeter': 0.01,
    'Kilonewton meter': 1000.0,
    'Gram-force millimeter': 0.00000980665,
  ...
  void _calculateConversion() {
    double rateFrom = torqueConversionRates[fromUnit] ?? 1.0;
    double rateTo = torqueConversionRates[toUnit] ?? 1.0;
  ...
  void _showUnitPicker(bool isFrom) async {
    ...
    final selectedUnit = await showModalBottomSheet<Map<String, String>>(
      ...
      builder: (context) {
        return const UnitSelectorSheet(category: 'Torque');
      },
    );

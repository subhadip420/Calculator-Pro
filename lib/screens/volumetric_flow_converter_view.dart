...
class _VolumetricFlowConverterViewState extends State<VolumetricFlowConverterView> {
  ...
  String fromUnit = 'Liter / Minute';
  String fromSymbol = 'l/min';
  String fromValue = '1';

  String toUnit = 'Gallon (US) / Minute';
  String toSymbol = 'gal/min';
  String toValue = '0.264172';

  late TextEditingController _fromController;
  late TextEditingController _toController;

  // Insert exactly the volumetricFlowConversionRates map provided above here!
  final Map<String, double> volumetricFlowConversionRates = {
    'Liter / Minute': 1.0,
    'Liter / Second': 60.0,
    'Gallon (US) / Minute': 3.78541178, // ... (Baki list add kar lena)
  };
  ...
  void _calculateConversion() {
    double rateFrom = volumetricFlowConversionRates[fromUnit] ?? 1.0;
    double rateTo = volumetricFlowConversionRates[toUnit] ?? 1.0;
  ...
  void _showUnitPicker(bool isFrom) async {
    ...
    final selectedUnit = await showModalBottomSheet<Map<String, String>>(
      ...
      builder: (context) {
        return const UnitSelectorSheet(category: 'Volumetric Flow');
      },
    );
